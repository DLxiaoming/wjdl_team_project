...
def main():
    ...
    # 中间点位姿（初始位置和插入位置之间的过渡点）
    # 使用初始位置和插入位置之间的中间值
    final_position = [0.42846348634645914, -0.41378344010325746, 1.014687063211261,
                     0.7142029141682644, -0.6799483429248572, -0.1424638597935098, -0.08537269469821417]
    # 计算中间点（位置取中间值，姿态使用初始姿态作为安全过渡）
    intermediate_point = [
        (init_pose[0] + final_position[0]) / 2,
        (init_pose[1] + final_position[1]) / 2,
        (init_pose[2] + final_position[2]) / 2,
        *init_pose[3:]  # 使用初始位置的姿态
    ]
    ...
    def execute_grasp(point_fdp_camera_xyz):
        ...
        logging.info("[1/10] 移动到准备位置（物体上方15cm）...")
        ...
        # 使用调整后的抓取姿态下降到抓取位置
        point_grasp_base = list(point_grasp_base_xyz) + list(grasp_rot_z)
        mmk2.control_arm_pose('left_arm', point_grasp_base)
        # time.sleep(1.5)
        

        # 步骤4：关闭夹爪抓取
        logging.info("[4/10] 关闭夹爪...")
        mmk2.set_robot_eef('left_arm', 0.2)  # 关闭夹爪
        # time.sleep(1.5)
        
        # 步骤5：返回到准备位置
        logging.info("[5/10] 返回到准备位置...")
        mmk2.control_arm_pose('left_arm', point_prepare_base)
        # time.sleep(2.0)
        
        # 步骤6：返回到初始位置
        logging.info("[6/10] 返回到初始位置...")
        mmk2.control_arm_pose('left_arm', init_pose)
        # time.sleep(2.0)
        # mmk2.set_robot_eef('left_arm', 1)  # 打开夹爪
        # mmk2.set_robot_eef('left_arm', 0)  # 关闭夹爪
        intermediate_point2 = [0.4688635562121496, -0.1779290959596237, 1.1563139387625228, -0.3840669784472783, 0.48499322614054635, 0.668519472560707, -0.41274185815266756]
        intermediate_point1 = [0.5197450340474683, -0.24490088302702248, 1.1007980271232107, -0.6414587300965526, 0.7027126750484081, 0.30400719738243637, -0.048012684250153474]   

        # 步骤7：移动到中间点（过渡到插入位置）
        # logging.info("[7/10] 移动到中间点（过渡到插入位置）...")
        # mmk2.control_arm_pose('left_arm', intermediate_point)
        # # time.sleep(1.5)

        # 步骤8：移动到插入位置
        logging.info("[8/10] 移动到插入位置...")
        mmk2.control_arm_pose('left_arm', final_position)
        mmk2.set_robot_eef('left_arm', 1)  # 打开夹爪，放物体

        # 步骤9：返回到中间点（过渡到初始位置）
        logging.info("[9/10] 返回到中间点（过渡到初始位置）...")
        mmk2.control_arm_pose('left_arm', intermediate_point1)
        # time.sleep(1.5)
        mmk2.control_arm_pose('left_arm', intermediate_point2)
        # 步骤10：返回到初始位置
        logging.info("[10/10] 返回到初始位置...")
        mmk2.control_arm_pose('left_arm', init_pose)        
