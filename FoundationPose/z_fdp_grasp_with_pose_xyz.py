...
def main():
    ...
    def execute_grasp(point_fdp_camera_xyz, quat_fdp_camera):
        ...
        logging.info(f"抓取姿态（使用FDP完整位姿）: [{quat_grasp_base[0]:+.4f}, {quat_grasp_base[1]:+.4f}, {quat_grasp_base[2]:+.4f}, {quat_grasp_base[3]:+.4f}]")
        ...
        # 步骤1：移动到准备位置（使用FDP完整位姿）
        logging.info("[1/5] 移动到准备位置（物体上方15cm，使用FDP完整6D位姿）...")
        mmk2.set_robot_eef('left_arm', 1)  # 打开夹爪
        point_prepare_base = list(point_prepare_base_xyz) + list(quat_grasp_base)  # 使用FDP完整姿态
        ...
        point_grasp_base = list(point_grasp_base_xyz) + list(quat_grasp_base)  # 使用FDP完整姿态
