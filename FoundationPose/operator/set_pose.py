...
if __name__ == "__main__":
    ...
    # mmk2.set_robot_spine(0.21481425)

    # init = [0.5215151530417846, -0.050107193701292074, 0.8469484341261568, 0.01249564960787811, 0.012812921441849817, -0.6253877332177554, 0.78010888401925]
    # mmk2.control_arm_pose('left_arm', init)
    
    init2 = [0.6071971944073851, 0.15423178989249392, 0.9227445894164919, 0.11923737406360825, 0.1825040434325132, -0.6101763468285463, 0.7616820521242235]
    mmk2.control_arm_pose('left_arm', init2)

    # mmk2.set_robot_eef('left_arm', 1)
    grasp = [0.6071971944073851, 0.15423178989249392, 0.9227445894164919, 0.010351212968842217, 0.07404863192388889, -0.4628662680264759, 0.8832691947665313]
    # mmk2.control_arm_pose('left_arm', grasp)
    # mmk2.set_robot_eef('left_arm', 0)
    safe_point = [0.7, 0.2, 0.9]
    end = [0.8219137684732842, 0.2165218772179783, 1.04292169762169, 0.9934553949641332, 0.06257672309462979, 0.09442909409380326, 0.014617733477523414]
    # mmk2.control_arm_pose('left_arm', end)

    from trajectory_plan_arm import run
    point_list = [
        grasp[0:3],
        safe_point,
        end[0:3],
    ...
    _, frames = run(point_list, x_dir)
    print("frames: ", frames)
    print("frames[0]: ", frames[0])
    print("frames[-1]: ", frames[-1])

    mmk2.control_arm_pose_waypoints('left_arm', [frames[0],frames[10]])

    # grasp = [0.699 ,0.042 ,0.902 ,0,   0,    0,    1   ]
    # frames[0] = [0.699,  0.042 , 0.902 , 0,    0,   0.707, -0.707]
    # frames[0] = [0.699,  0.042 , 0.902 , 1,    0,   0, 0]
    # frames[0] = [0.6071971944073851, 0.15423178989249392, 0.9227445894164919 , 0,    0,   0.707, -0.707]
    print("------------")
    # print(f"{frames[0]}")
    # mmk2.control_arm_pose('left_arm', frames[0])

    
    # print(f"{frames[50]}")
    # mmk2.control_arm_pose('left_arm', frames[20])
    # mmk2.control_arm_pose('left_arm', frames[80])


    # grasp = [0.822, 0.217, 1.043 ,1,    0,    0,    0  ]
    # frames[-1] = [0.822 , 0.217  ,1.043, 1,    0,   0, 0]
    # print(f"{frames[-1]}")
    # mmk2.control_arm_pose('left_arm', frames[-1])

    # frame2 = [f for i,f in enumerate(frames) if i%10 ==0]
#     frame2 = frames
#     print("frame2: ", frame2)
#     for i, f in enumerate(frame2):
#         print(f"{i} : ", f)
#         try:
#             mmk2.control_arm_pose('left_arm', f)
#             print("success")
#         except:
#             pass
#         # mmk2.control_arm_pose('left_arm', f)
