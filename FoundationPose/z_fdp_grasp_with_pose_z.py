...
def main():
    ...
    parser = argparse.ArgumentParser(description='FDP自动检测抓取（使用完整位姿）')
    ...
    def detect_fdp():
        ...
        for img_head, img_depth, _, _ in camera:
            ...
            try:
                ...
                return point_fdp_camera_xyz, obj_quat_camera, vis
    ...
    def execute_grasp(point_fdp_camera_xyz, quat_fdp_camera):
        """执行抓取流程（Z轴垂直，XY旋转使用FDP检测的）"""
        print("\n" + "="*70)
        print("🤖 开始执行抓取流程（Z轴垂直，XY旋转使用FDP）...")
        print("="*70)
        ...
        # 提取FDP检测的yaw角度（Z轴旋转，XY平面内的旋转）
        euler_fdp_base = Rotation.from_quat(quat_fdp_base).as_euler('xyz', degrees=False)
        yaw_angle = euler_fdp_base[2]  # Z轴旋转角度
        
        # 构造新姿态：Z轴垂直（pitch=0, roll=0），但保持FDP的yaw角度
        # 这会让机械臂垂直向下抓取，但XY平面内的旋转与FDP检测一致
        quat_grasp_base = Rotation.from_euler('xyz', [0.0, 0.0, yaw_angle]).as_quat()
        ...
        logging.info(f"抓取姿态（Z轴垂直，yaw={np.degrees(yaw_angle):.1f}°）: [{quat_grasp_base[0]:+.4f}, {quat_grasp_base[1]:+.4f}, {quat_grasp_base[2]:+.4f}, {quat_grasp_base[3]:+.4f}]")
        ...
        # 步骤1：移动到准备位置（Z轴垂直，XY旋转使用FDP）
        logging.info("[1/5] 移动到准备位置（物体上方15cm，Z轴垂直，XY旋转与FDP一致）...")
        mmk2.set_robot_eef('left_arm', 1)  # 打开夹爪
        point_prepare_base = list(point_prepare_base_xyz) + list(quat_grasp_base)  # 使用混合姿态
        ...
        point_grasp_base = list(point_grasp_base_xyz) + list(quat_grasp_base)  # 使用混合姿态
    ...
    while True:
        # 执行FDP检测
        point_fdp_camera_xyz, quat_fdp_camera, vis = detect_fdp()
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                execute_grasp(point_fdp_camera_xyz, quat_fdp_camera)
                # 抓取完成后，等待用户操作
                while True:
                    key = cv2.waitKey(100) & 0xFF
                    if key == ord('q'):
